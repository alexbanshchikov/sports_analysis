from rest_framework.routers import DefaultRouter

from .views import AmpluaViewSet, CoachViewSet, CoachInTeamViewSet, ConferenceViewSet, DerbiViewSet, DivisionViewSet, \
    GameViewSet, GameGoalsViewSet, GamePenaltiesViewSet, GameStatsViewSet, GoalkeeperStatsViewSet, LeagueViewSet, \
    MannerOfPlayViewSet, MannerOfPlayInTeamViewSet, OffenceStatsViewSet, PersonalEnemyViewSet, PlayerViewSet, \
    PlayerInTeamViewSet, PlayerStatusViewSet, RefereeViewSet, RefereeInGameViewSet, SeasonViewSet, TeamViewSet, \
    TeamAdditionalStatsViewSet, TeamAfterGameBullitsViewSet, TeamGoalShootsViewSet, TeamInConferenceViewSet, \
    TeamInDivisionViewSet, TeamMainStatsViewSet, TeamPenaltiesViewSet, TeamStatsByImplementedGoalsViewSet, \
    TeamStatsByMissedGoalsViewSet, TeamStatsByPeriodViewSet, TeamStatsByStrengthViewSet, TimePeriodViewSet, \
    TypeOfGameViewSet

router = DefaultRouter()
router.register(r'hockey_app/amplua', AmpluaViewSet)

router.register(r'hockey_app/coach', CoachViewSet)
router.register(r'hockey_app/coach_in_team', CoachInTeamViewSet)

router.register(r'hockey_app/conference', ConferenceViewSet)

router.register(r'hockey_app/derbi', DerbiViewSet)

router.register(r'hockey_app/division', DivisionViewSet)

router.register(r'hockey_app/game', GameViewSet)
router.register(r'hockey_app/game_goals', GameGoalsViewSet)                                         # router.register(r'hockey_app/game/goals', GameGoalsViewSet)
router.register(r'hockey_app/game_penalties', GamePenaltiesViewSet)                                 # router.register(r'hockey_app/game/penalties', GamePenaltiesViewSet)
router.register(r'hockey_app/game_stats', GameStatsViewSet)                                         # router.register(r'hockey_app/game/stats', GameStatsViewSet)

router.register(r'hockey_app/goalkeeper_stats', GoalkeeperStatsViewSet)

router.register(r'hockey_app/league', LeagueViewSet)

router.register(r'hockey_app/manner_of_play', MannerOfPlayViewSet)
router.register(r'hockey_app/manner_of_play_in_team', MannerOfPlayInTeamViewSet)

router.register(r'hockey_app/offence_stats', OffenceStatsViewSet)

router.register(r'hockey_app/personal_enemy', PersonalEnemyViewSet)

router.register(r'hockey_app/player', PlayerViewSet)
router.register(r'hockey_app/player_in_team', PlayerInTeamViewSet)
router.register(r'hockey_app/player_status', PlayerStatusViewSet)                                   # router.register(r'hockey_app/player/status', PlayerStatusViewSet)

router.register(r'hockey_app/referee', RefereeViewSet)
router.register(r'hockey_app/referee_in_game', RefereeInGameViewSet)

router.register(r'hockey_app/season', SeasonViewSet)

router.register(r'hockey_app/team', TeamViewSet)
router.register(r'hockey_app/team_additional_stats', TeamAdditionalStatsViewSet)                    # router.register(r'hockey_app/team/additional_stats', TeamAdditionalStatsViewSet)
router.register(r'hockey_app/team_after_game_bullits', TeamAfterGameBullitsViewSet)                 # router.register(r'hockey_app/team/after_game_bullits', TeamAfterGameBullitsViewSet)
router.register(r'hockey_app/team_goal_shoot', TeamGoalShootsViewSet)                               # router.register(r'hockey_app/team/goal_shoot', TeamGoalShootsViewSet)
router.register(r'hockey_app/team_in_conference', TeamInConferenceViewSet)
router.register(r'hockey_app/team_in_division', TeamInDivisionViewSet)
router.register(r'hockey_app/team_main_stats', TeamMainStatsViewSet)                                # router.register(r'hockey_app/team/main_stats', TeamMainStatsViewSet)
router.register(r'hockey_app/team_penalties', TeamPenaltiesViewSet)                                 # router.register(r'hockey_app/team/penalties', TeamPenaltiesViewSet)
router.register(r'hockey_app/team_stats_by_implemented_goals', TeamStatsByImplementedGoalsViewSet)  # router.register(r'hockey_app/team/stats_by_implemented_goals', TeamStatsByImplementedGoalsViewSet)
router.register(r'hockey_app/team_stats_by_missed_goals', TeamStatsByMissedGoalsViewSet)            # router.register(r'hockey_app/team/stats_by_missed_goals', TeamStatsByMissedGoalsViewSet)
router.register(r'hockey_app/team_stats_by_period', TeamStatsByPeriodViewSet)                       # router.register(r'hockey_app/team/stats_by_period', TeamStatsByPeriodViewSet)
router.register(r'hockey_app/team_stats_by_strength', TeamStatsByStrengthViewSet)                   # router.register(r'hockey_app/team/stats_by_strength', TeamStatsByStrengthViewSet)

router.register(r'hockey_app/time_period', TimePeriodViewSet)

router.register(r'hockey_app/type_of_game', TypeOfGameViewSet)

urlpatterns = router.urls
